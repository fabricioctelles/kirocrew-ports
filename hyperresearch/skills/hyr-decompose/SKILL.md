...
name: hyr-decompose
description: >
  Step 1 of HyperResearch for KiroCrew. Decomposes the research query into
  atomic items, classifies pipeline_tier and response_format, and produces
  the coverage matrix that downstream steps depend on.
triggers: decompose query, hyr decompose, hyperresearch step 1, research decomposition
---
...
**Goal:** before any research happens, decompose the user's prompt into its atomic items. This artifact is read by downstream steps to make sure the pipeline doesn't drift from what was actually asked.
...
The orchestrator's bootstrap step has already produced:
...
1. **Re-read the canonical research query** end to end (`~/.kiro/crew/workspace/research/runs/<vault_tag>/query.md`).
...
4. **Write `~/.kiro/crew/workspace/research/runs/<vault_tag>/prompt-decomposition.json`:**
...
6. **Do NOT include wrapper-contract requirements here** — those live in `~/.kiro/crew/workspace/research/wrapper_contract.json` separately. The decomposition is ONLY about what the user's actual prompt named.
...
7. **Classify `pipeline_tier` and `response_format`.**
   ...
   **`citation_style`** — how the final report handles source attribution:
   ...
   |-------|-------------|--------|
   | `"wikilink"` | **Default.** Personal use — every citation is a clickable link back to the raw source note in `~/.kiro/crew/workspace/research/notes/`. | `[[note-id]]` markers inline. No separate Sources section (each link self-resolves to the source note's frontmatter title + URL). |
   | `"inline"` | Public deliverable or verifiable research report for someone outside the vault. | `[N]` inline citations (grouped `[7, 12]` when one point cites several sources) + a formatted `## Sources` list at the end. |
   | `"none"` | Polished expert-analysis with no visible citation apparatus. | No citation markers, no Sources section. |

   **Wrapper override:** if `~/.kiro/crew/workspace/research/wrapper_contract.json` exists and specifies `citation_style`, it overrides the default.

   **`levers`** — run-time posture, auto-selected here. Three fields:
   ...
   **Precedence:** an explicit user directive in the query ("make it a survey", "just teach me", "mode=teach") ALWAYS wins over your classification. If `~/.kiro/crew/workspace/research/wrapper_contract.json` has a `levers` key, it wins over classification but not over explicit user text.
   ...
   **`inference_depth`** — `"surface"` (bounded question, authoritative consensus suffices), `"standard"` (default), or `"deep"` (the answer likely lives beyond the clear web: gray literature, filings, inference over absences). This is PROVISIONAL — later steps may re-evaluate it.
...
8. **Coverage matrix self-audit.** Re-read the verbatim query. Walk through it phrase by phrase and extract every **significant noun phrase, proper noun, technical term, and category name**. For each:
   ...
   Write the matrix to `~/.kiro/crew/workspace/research/runs/<vault_tag>/temp/coverage-matrix.md`:
...
9. **Update the scaffold.** Append a "Tier rationale" subsection to `~/.kiro/crew/workspace/research/runs/<vault_tag>/scaffold.md` with a 2-3 sentence justification for the tier classification.

10. **Render the lever shims.** Create `~/.kiro/crew/workspace/research/runs/<vault_tag>/shims/` directory and write four shim files (`research.md`, `drafting.md`, `critics.md`, `polish.md`) from the levers block. Each shim includes:
    - The register and its implications for voice
    - The domain_notes verbatim
    - The inference_depth and what it means for sourcing

    Example `research.md` shim:
    ```markdown
    ## Research Posture

    **Register:** analyze (evaluation-shaped)
    **Inference depth:** standard
    **Domain notes:** Sourcing: academic APIs first (arXiv, Nature); recency matters within 24 months for hardware claims; measured figures outrank vendor claims.

    Implication: prioritize quantitative evidence, comparative frameworks, and authoritative sources. Evaluate competing claims rather than merely cataloging them.
    ```
...
- `~/.kiro/crew/workspace/research/runs/<vault_tag>/prompt-decomposition.json` exists, is valid JSON, every atomic item traces to the research_query
...
- All four shim files exist under `~/.kiro/crew/workspace/research/runs/<vault_tag>/shims/`
- `~/.kiro/crew/workspace/research/runs/<vault_tag>/temp/coverage-matrix.md` exists with **zero `Gap? = YES` rows**
- `~/.kiro/crew/workspace/research/runs/<vault_tag>/scaffold.md` includes a Tier rationale subsection

---

## Next step

Return to the orchestrator. Read `~/.kiro/crew/workspace/research/runs/<vault_tag>/prompt-decomposition.json` to learn the tier, then proceed to step 2 (width sweep).
